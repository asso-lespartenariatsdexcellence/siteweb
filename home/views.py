...
def actionscampus(request):
    return render(request, "home/actionscampus.html", getBase(request))


def college(request):
    cours = Cours.objects.filter(estActionCollege=True)
    if not request.user.is_authenticated:
        cours.filter(public=True)
    return render(request, "home/college.html", getBase(request) | {'cours': cours})


def lycee(request):
    cours = Cours.objects.filter(estActionLycee=True)
    if not request.user.is_authenticated:
        cours.filter(public=True)
    return render(request, "home/lycee.html", getBase(request) | {'cours': cours})
...
def notrecampus(request):
    ...
    if not request.user.is_authenticated:
        cours.filter(public=True)
    return render(request, "home/notrecampus.html", getBase(request) | {'cours': cours})


def nous(request):
    return render(request, "home/nous.html", getBase(request))


def histoire(request):
    return render(request, "home/histoire.html", getBase(request))


def partenaires(request):
    return render(request, "home/partenaires.html", getBase(request))


def sdl(request):
    return render(request, "home/sdl.html", getBase(request))
